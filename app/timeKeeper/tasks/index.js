...
export default function EmployeeDocsScreen() {
    ...
    useFocusEffect(useCallback(() => {
        request({
            url: `/timekeeper/options/task_statuses`, method: 'get', cache: true,
        }).then().catch(err => {
            console.log(err);
        })

        request({
            url: '/timekeeper/project/tasks', method: 'get',
        }).then(res => {
    ...
    useEffect(() => {
        setTaskList(storeData?.cache?.[`GET:/timekeeper/project/tasks`]?.data)
    }, [storeData?.cache?.[`GET:/timekeeper/project/tasks`]])
